...
?>
	...
	<div class="notice exemple">Ex : Date, Numéro de facture, Numéro du vendeur, Numéro du produit, Quantité vendue, Prix unitaire, Description du produit,
     Sous total, Taxes, Total ...</div>
...
<table class="table" summary="Données d'une base de données relationnelle">
 ...
 <td>Qté commandée</td>
	...
	<p>Il en va de même pour les informations des tables <strong>VENDEUR</strong> et <strong>Article</strong> reliées à la table <strong>FACTURE</strong> par leurs champs communs respectifs <strong>N&deg; VENDEUR</strong> et <strong>N&deg; PRODUIT</strong> .</p>
	...
	<h2>Définitions</h2>
	<p>Il est important de bien maîtriser le vocabulaire lié à l'utilisation d'une base de données. Ces nouveaux termes seront utilisés tout au long des pages concernant OpenOffice Base.</p>
<table class="table" summary="Définitions des termes relatifs à OpenOffice Base">
 <tr>
 <th class="w200p" >Champ</th>
 ...
 <td>Présentation d'une partie ou de l'ensemble des données sous un format imprimable.</td>
	...
	<p>La définition d'une base de données prend maintenant un sens plus large qu'auparavant. Ce n'est plus juste un fichier qui contient de l'information utile pour l'utilisateur. Il contient aussi les requêtes, les formulaires, les rapports, les macros et les modules pour développer une &quot;application&quot; ou un &quot;système&quot; qui répond aux besoins spécifiques de l'utilisateur.</p>
	
	<h2>Quelles sont les composantes d'OpenOffice Base ?</h2>
	<p><img src="../img/oobase/bt_tables.png" alt="bouton tables" />Contient les champs recueillant les informations. C'est l'objet fondamental de la base de données.</p>
	<p><img src="../img/oobase/bt_requetes.png" alt="bouton requêtes" />Recherche d'information qui répond à certains critères déterminés par l'utilisateur.</p>
	<p><img src="../img/oobase/bt_formulaires.png" alt="bouton formulaires" />Présentation de l'information à l'écran d'une manière conviviale pour l'utilisateur.</p>
    <p><img src="../img/oobase/bt_rapports.png" alt="bouton rapports" />Présentation imprimable de l'information.</p>
	
	<h2>Création d'une base de données</h2>
	<p>Pour vous aider à comprendre la création d'une base de données, nous allons montrer les étapes à suivre. Afin d'éviter des répétitions, vous serez amenés à lire d'autres parties du cours, notamment sur <a href="/cours/qlio/oobase2a">les tables</a> et <a href="/cours/qlio/oobase3a">les relations</a>. L'exercice va consister à créer une base de données pour la gestion d'un commerce.</p>
	<p>La première étape de la création d'une base de données est l'analyse. C'est d'ailleurs probablement l'étape la plus importante. Si vous passez trop rapidement sur cette étape, vous allez ensuite perdre beaucoup de temps et d'efforts à refaire ce qui aurait d&ucirc; être accompli auparavant.</p>
	<p>Avant de créer votre base de données, il faut avoir une idée claire des informations dont vous aurez besoin.</p>
	<div class="notice exemple">Ex : informations sur mes clients, sur mes fournisseurs, sur mon inventaire, sur mon personnel, etc.</div>
    <p>Une des techniques que vous pouvez utiliser consiste à partir des formulaires et des rapports dont vous aurez besoin. Connaissant les résultats, vous découvrirez quels sont les champs nécessaires pour obtenir ceux-ci.</p>
	<div class="notice exemple">Ex : il faut connaître les quantités vendues et à quel prix unitaire avant de connaître le total des ventes.</div>
    <p>Vous pouvez également partir de la base de données et déterminer les informations qui sont nécessaires pour les requêtes, les formulaires et les rapports.</p>
	...
	<div class="notice exemple">Ex : est-ce que le champ &quot;<strong>Escompte</strong>&quot; ou &quot;<strong>Conditions de paiement</strong>" ira dans l'entité (table) &quot;<strong>Facture</strong>&quot; ou &quot;<strong>Client</strong>&quot; ?</div>
	...
	<ul>
	 ...
	 <div class="notice exemple">Ex : Total = Qté * Prix unitaire</div>
	...
	<p>&Agrave; l'exception des champs en commun qui servent à &quot;relier&quot; les tables, un champ ne devrait pas se retrouver dans plusieurs tables.</p>
	
    <h3>Déterminer les clés primaires</h3>
	<p>Une clé primaire est un champ (ou une série de champs) qui permet de différencier un enregistrement des autres.</p>
	<div class="notice exemple">Ex : bien qu'il puisse y avoir plusieurs factures avec la même date, la même quantité achetée, le même client ou le même vendeur, il n'y aura pas deux factures avec le même numéro de facture.</div>
	<p>Une clé primaire n'est pas obligatoire pour une table. Elle le devient lorsque vous voulez relier deux tables.</p>
	<p>Essayez de déterminer le champ approprié pour une clé primaire pour la table <strong>Employe</strong>.</p>
	<p>Certainement pas le champ <strong>sexe</strong>. &Agrave; moins qu'il y ait seulement un homme et une femme dans l'entreprise. Cela limite un peu la croissance de l'entreprise !</p>
	<p>Il serait possible d'utiliser le champ <strong>Nom de famille</strong> tant que deux personnes dans l'entreprise n&rsquo;ont pas le même nom. Pour régler le problème, il est possible de créer une clé primaire composé de deux champs&nbsp;: <strong>Nom de famille</strong> et <strong>emp_prenom</strong>.</p>
	...
	<p>Prenez quelques instants pour essayer de déterminer quels champs dans la table seraient appropriés pour être la clé primaire ou être dans la clé primaire. L'explication pour <a href="/cours/qlio/oobase2a#chap33">appliquer la clé primaire</a> sur un ou plusieurs champs de la table se trouve dans le cours suivant sur les tables.</p>
	...
	<ul>
		...
		<li>Le même type de champ (<strong>Texte</strong> avec texte, <strong>numérique</strong> avec <strong>numérique</strong>... sauf exception de <strong>Interger [INTEGER]</strong> avec <strong>numérique</strong>).</li>
	...
	<p>Vous n'êtes pas obligé de relier toutes les tables les unes aux autres. Il est possible d'accéder à une information d'une des tables si les tables sont reliées soit directement, soit indirectement entre elles. Vous trouverez plus de détails sur la page des <a href="/cours/qlio/oobase3a">relations</a>.</p>
	...
	<p>Pour déterminer le type de relation et comment créer des relations vous trouverez de plus amples explications dans le cours sur les <a href="/cours/qlio/oobase3a">relations</a>.</p>
	<p>Dans le cas d'une facturation, nous pourrions relier les tables de la façon suivante&nbsp;:</p>
		<img src="../img/oobase/rel_toutes_tables.png" alt="relations entre toutes les tables" />
	...
	<p>La relation de types <strong>plusieurs à plusieurs</strong> requiert une table intermédiaire composée, au minimum de la clé primaire des deux tables à relier. C'est la raison d'être de la table <strong>Transition_Fact-Stock</strong>.</p>
	...
	<div class="notice exemple">Ex : une facture peut contenir plusieurs produits.</div>
 ...
 <tr class="middle">
  ...
  <td>1, 3, 5</td>
	...
	<p>En effet, si nous devions modifier un numéro de produit, nous devrions répercuter cette modification sur l'ensemble des enregistrements de toutes les factures dans la table <strong>Facture</strong>. Vous convenez que ce travail est fastidieux et peut être source d&rsquo;erreurs. Nous devons donc placer le champ <strong>ID_Produit</strong> dans une table autre que <strong>Facture</strong>.</p>
	...
	<div class="notice exemple">Ex : cela ne serait pas efficace d'avoir une table &quot;<strong>Facture</strong>&quot; qui contienne également les champs &quot;<strong>Nom du client</strong>&quot;,
     &quot;<strong>Adresse de livraison</strong>&quot;, &quot;<strong>personne contact</strong>&quot;.</div>
	...
	<ul>
		<li><strong>Facture</strong> : ID_Facture, date, bon de commande, escompte.</li>
		<li><strong>Client</strong> : ID_Client, adresse de facturation, ville, numéro de téléphone, numéro de télécopieur, <strong>adresse</strong> de courriel, adresse de livraison, personne contact, escompte, conditions de paiement.</li>
		<li><strong>Employe</strong> : ID_Vendeur, nom, prénom, poste, bureau, salaire, commission, embauche.</li>
		<li><strong>Stock</strong> : ID_Produit, description, prix unitaire, quantité disponible.</li>
	</ul>
	<p>Nous devons ensuite déterminer la clé primaire pour chaque table. Cela est nécessaire pour la seconde forme normale. Une clé primaire est un champ (ou une série de champs) qui permet de distinguer un enregistrement des autres. Pour la table <strong>Facture</strong>, la clé primaire sera le champ <strong>ID_Facture</strong>.</p>
	...
	<ul>
		<li><strong>Facture</strong> : ID_Facture, Date, bon de commande, escompte.</li>
		<li><strong>Client</strong> : ID_Client, adresse de facturation, ville, numéro de téléphone, numéro de télécopieur, adresse de courriel, adresse de livraison, personne contact, escompte, conditions de paiement.</li>
		<li><strong>Employe</strong> : ID_Vendeur, nom, prénom, poste, bureau, salaire, commission, embauche.</li>
		<li><strong>Stock</strong> : ID_Produit, description, prix unitaire, quantité disponible.</li>
		<li><strong>Transition_Fact-Stock</strong> : ID_Facture, ID_Produit, quantité achetée.</li>
	</ul>
	<div class="notice exemple">Ex : la table <strong>Transition_Fact-Stock</strong> joue le rôle d'intermédiaire entre les tables <strong>Facture</strong> et <strong>Stock</strong> pour éviter une saisie
     redondante et surtout permettre une relation entre les clés primaires des deux tables.</div>
	<p>Vous remarquez que la clé primaire de cette table est composé de deux champs&nbsp;: <strong>fact_id</strong> et <strong>Numéro de produit</strong>. Ce sont aussi les clés primaires des tables <strong>Facture</strong> et <strong>Stock</strong>.</p>
 ...
 <tr class="middle">
  ...
  <td>100</td>
	...
	<p>Pour la troisième forme normale, il s'agit d'éliminer les dépendances transitives. C&rsquo;est-à-dire qu'il faut s'assurer qu'il n'y ait pas de tables qui soient cachées parmi les autres.</p>
	<p>De cette manière, les tables ne devraient jamais contenir de champs calculés.</p>
	<div class="notice exemple">Ex : les champs &quot;<strong>Sous-total</strong>&quot;, &quot;<strong>Total</strong>&quot; ou &quot;<strong>TVA</strong>&quot; ne devraient pas apparaître dans les tables puisqu'il est possible de les
     ...
     Donc, il est inutile de le saisir dans les tables.</div>
 ...
 Une fois que vous avez réalisé les regroupements et déterminé les relations, vous avez votre base pour la création des tables.</p>
    <p>Maintenant que vous avez les entités et les champs qui les composent, imaginez à quoi devrait ressembler vos formulaires et vos rapports. Est-ce que les champs que vous avez choisis répondent à tous vos besoins&nbsp;?</p>
    ...
    <p>Maintenant que vous avez créé la base de données, il ne reste qu'à créer les tables, saisir les données, créer les requêtes, les formulaires, les rapports, les macros et les modules pour votre base de données.</p>
