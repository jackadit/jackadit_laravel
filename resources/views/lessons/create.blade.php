...
@section('content')
    <div class="container mx-auto px-4 py-6">
        ...
        <div class="bg-white rounded-lg shadow-md overflow-hidden">
            ...
            <!-- Formulaire -->
            <div class="p-8">
                <form action="{{ route('lessons.store', $course) }}"
                      method="POST"
                      class="space-y-6">
                    @csrf
                    ...
                    <div>
                        ...
                        <textarea name="description"
                                  ...
                                  rows="4"
                                  class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all"
                                  placeholder="Décrivez le contenu de la leçon...">{{ old('description') }}</textarea>
                        @error('description')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div id="video_url_field" style="display: {{ old('content_type') == 'video' ? 'block' : 'none' }};">
                    ...
                    <div id="content_field" style="display: {{ old('content_type') == 'text' ? 'block' : 'none' }};">
                        ...
                        <textarea name="content"
                                  ...
                                  placeholder="Écrivez le contenu de votre leçon...">{{ old('content') }}</textarea>
                        @error('content')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="number"
                                   name="order"
                                   id="order"
                                   value="{{ old('order', $course->lessons()->max('order') + 1) }}"
                                   required
                                   min="1"
                                   class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all"
                                   placeholder="Ex: 1">
                            <p class="mt-2 text-xs text-gray-500">
                                Position actuelle : {{ $course->lessons()->count() + 1 }}
                            </p>
                            @error('order')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
